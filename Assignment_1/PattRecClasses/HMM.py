...
class HMM:
    ...
    1: a State Sequence Generator of type MarkovChain (mc)
    2: an array of output probability distributions, one for each state (distribution)
    ...
    def __init__(self, mc, distributions):
        ...
        self.stateGen = mc
        self.outputDistr = distributions # g3 = [g1, g2] 

        self.nStates = mc.nStates
        self.dataSize = distributions[0].dataSize # judge whether Single_Gaussian or Mixed_Gaussian
    ...
    def rand(self, nSamples):
        ...
        S = self.stateGen.rand(nSamples) # if finite, len(S) <= nSamples
        x_len = len(S) #number of states
        X = np.empty((self.dataSize, x_len))
        for t in range (x_len):
            curr_state = S[t]
            X[:, t] = self.outputDistr[curr_state].rand(1).ravel()                
        return X, S
